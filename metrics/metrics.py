...
class GameMetrics:
    ...
    def record_move(
        self, agent_name: str, time_taken: float, is_legal: bool = True
    ) -> None:
        """
        ...
        Args:
            agent_name(str): Identifier for the agent.
            time_taken (float): Time taken for the move.
            is_legal (bool): Whether the move was legal.
        """
        if agent_name not in self.agents:
    ...
    def record_result(self, agent_name: str, result: str) -> None:
        """
    ...
    def plot_performance_radar(self, save_path=None):
        ...
        for agent in self.agents:
            ...
            max_vals = [1.0, 1.0, max(1.0, speed)]
    ...
    def plot_results(self, save_path: str = None, show: bool = True):
        ...
        plt.subplot(2, 2, 1)
        total_games = [
            self.agents[agent]["wins"]
            + self.agents[agent]["losses"]
            + self.agents[agent]["draws"]
            for agent in agent_names
        ]
        win_percentages = [
            (
                self.agents[agent]["wins"] / total_games[i] * 100
                if total_games[i] > 0
                else 0.0
            )
        ...
        loss_percentages = [
            (
                self.agents[agent]["losses"] / total_games[i] * 100
                if total_games[i] > 0
                else 0.0
            )
            for i, agent in enumerate(agent_names)
        ...
        draw_percentages = [
            (
                self.agents[agent]["draws"] / total_games[i] * 100
                if total_games[i] > 0
                else 0.0
            )
            for i, agent in enumerate(agent_names)
        ...
        for i in range(len(agent_names)):
            plt.text(
                x[i] - width,
                win_percentages[i] + 1,
                f"{win_percentages[i]:.1f}%",
                ha="center",
                va="bottom",
            )
            plt.text(
                x[i],
                loss_percentages[i] + 1,
                f"{loss_percentages[i]:.1f}%",
                ha="center",
                va="bottom",
            )
            plt.text(
                x[i] + width,
                draw_percentages[i] + 1,
                f"{draw_percentages[i]:.1f}%",
                ha="center",
                va="bottom",
            )
    ...
    def __str__(self) -> str:
        ...
        for agent, stats in self.agents.items():
            ...
            report.append(
                f"  Games: {total_games} (W: {stats['wins']}, L: {stats['losses']}, D: {stats['draws']})"
            )
            ...
            report.append(
                f"  Total Moves: {total_moves} (Legal: {stats['legal_moves']}, Illegal: {stats['illegal_moves']})"
            )
        return "\n".join(report)
